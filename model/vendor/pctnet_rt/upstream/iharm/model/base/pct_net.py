...
class PCTNet(nn.Module):
    ...
    def __init__(
        ...
        self.affine = affine
        self.PCT = PCT(transform_type, dim, affine, color_space,
                       input_normalization['mean'], input_normalization['std'], clamp=clamp)
    ...
    def forward(self, image, image_fullres=None, mask=None, mask_fullres=None, backbone_features=None):
        ...
        latent, attention_map = self.decoder(intermediates, image, mask)  # (N, C, H, W), (N, 1, H, W)

        # Guard: ensure decoder outputs match input spatial size
        if latent.shape[2:] != image.shape[2:]:
            latent = F.interpolate(latent, size=image.shape[2:], mode='bilinear', align_corners=False)
        ...
        # Guard: if latent channels don't match the parameter head's expected input, project with 1x1 conv
        expected_c = self.get_params.in_channels  # e.g., 32, as in your checkpoint
        if latent.shape[1] != expected_c:
            # Create (or refresh) a projection layer lazily, then apply it
            proj_name = "_param_in_proj"
            need_new = True
            if hasattr(self, proj_name):
                p = getattr(self, proj_name)
                if isinstance(p, nn.Conv2d) and p.in_channels == latent.shape[1] and p.out_channels == expected_c:
                    need_new = False
            if need_new:
                setattr(self, proj_name, nn.Conv2d(latent.shape[1], expected_c, kernel_size=1).to(latent.device))
            latent = getattr(self, proj_name)(latent)
    
        params = self.get_params(latent)
        print("pct params mean abs:", float(params.abs().mean()))
        ...
        if torch.is_tensor(image_fullres):
            ...
            fr_imgs = [img_fr.unsqueeze(0).to(image.get_device()) for img_fr in image_fullres] if image_fullres is not None else []
            fr_masks = [mask_fr.unsqueeze(0).to(image.get_device()) for mask_fr in mask_fullres] if mask_fullres is not None else []
            idx = [[n] for n in range(len(fr_imgs))]
